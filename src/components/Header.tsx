...
export default function Header({
  ...
  const renderSyncIndicator = () => {
    ...
    switch (syncStatus) {
      ...
      case 'simulator':
        return (
          <div className="hidden lg:flex items-center gap-2.5 bg-sky-50 border border-sky-100 px-3 py-1.5 rounded-xl text-sky-700 font-bold text-[11px] h-9 shadow-sm cursor-help" title="Sinkronisasi otomatis dialihkan ke database server lokal karena kuota server awan terlampaui. Data Anda tetap tersimpan dengan aman!">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-sky-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-sky-500"></span>
            </span>
            <Cloud className="w-3.5 h-3.5 text-sky-600" />
            <span>Sinkronisasi Lokal Aktif {timeString && `(${timeString})`}</span>
            <button
              onClick={onManualSync}
              type="button"
              title="Coba segarkan koneksi awan"
              className="p-1 hover:bg-sky-100 rounded-lg text-sky-600 transition"
            >
              <RefreshCw className="w-3.5 h-3.5 hover:rotate-180 duration-500 transition-transform" />
            </button>
          </div>
        );
      case 'active':
