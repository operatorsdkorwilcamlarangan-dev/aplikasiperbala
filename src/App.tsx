          ...
          fetch('/api/local-db')
            ...
            .then((localData) => {
              if (localData && localData.success) {
                rawSetSchools(localData.schools || initialSchools);
                rawSetOperators(localData.operators || initialOperators);
                rawSetMonthlyPagu(localData.monthlyPagu || initialMonthlyPagu);
                rawSetRabList(localData.rabList || initialRAB);
                rawSetTransactions(localData.transactions || initialTransactions);
                rawSetTarikTunaiList(localData.tarikTunaiList || initialTarikTunai);
                rawSetSystemConfig(localData.systemConfig || defaultSystemConfig);
            ...
            .catch((backupErr) => {
              console.error('Failed to load from backup database server:', backupErr);
            });
        }
        return;
      }
    ...
    try {
      ...
      try {
        ...
        if (localData && localData.success) {
          ...
          if (isManual) {
            addToast('Sinkronisasi Sukses (Server Cadangan)', 'Data berhasil disinkronkan menggunakan server cadangan lokal.', 'success');
          }
        } else {
        ...
        setSyncErrorReason(errReason);
        if (isManual) {
          addToast('Koneksi Gagal', 'Gagal menyegarkan data terbaru dari server awan dan cadangan lokal.', 'error');
        }
      }
    }
  };
